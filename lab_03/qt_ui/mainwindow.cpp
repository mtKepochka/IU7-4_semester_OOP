...
void MainWindow::setupScene(QWidget *parent)
{
    ...
    std::shared_ptr<BaseDrawer> drawer = solution.create<QtFactory>(scene);
...
{
    logic->rotateCamera(meta->getActiveCamera(), 0, 1);
}
...
{
    logic->rotateCamera(meta->getActiveCamera(), 1, 0);
}
...
void MainWindow::btnLoadModelClicked()
{
    ...
    QString figName = QString("SceletonModel (") + QString::number(objectId)
...
void MainWindow::on_btn_create_composite_clicked()
{
    std::vector<int> ids;
    std::string ids_string = ui->composite_edit->text().toStdString();
    if (ids_string.empty())
        return;
    ...
    size_t pos = 0;
    std::string token;
    while ((pos = ids_string.find(delimiter)) != std::string::npos) {
        token = ids_string.substr(0, pos);
        try {
            ids.push_back(std::stoi(token));
        } catch (...) {

        }
        ids_string.erase(0, pos + delimiter.length());
    }
    try {
        ids.push_back(std::stoi(ids_string));
    } catch (...){
    ...
    {
        int id = *it;
        if (!meta->getObjects().contains(id))
            return;
        if (logic->hasCameraId(id))
            ids.erase(it);
        else
            it++;
    }
    ...
    for (auto id : ids)
    {
        ui->modelCB->removeItem(ui->modelCB->findData(QVariant(id)));
    }

    int composite_id = logic->createComposite(ids);

    QString figName = QString("SceletonModelComposite (") + QString::number(composite_id) + QString(")");

    ui->modelCB->addItem(figName, composite_id);
    ui->modelCB->setCurrentIndex(ui->modelCB->count() - 1);
